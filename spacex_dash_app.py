...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Successful Launches Count')
        return fig
    else:
        # return the outcomes piechart for a selected site
        data = spacex_df.groupby(['Launch Site', 'class']).size().reset_index(name='Count')
        fig = px.pie(data[data['Launch Site'] == entered_site], values='Count', 
        names='class', 
        title='Successful vs Failed Launches Count')
        return fig
# TASK 4:
